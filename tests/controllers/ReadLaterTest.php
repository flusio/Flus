...
class ReadLaterTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testBookmarksRedirectsToReadLaterWithQueryString(): void
    {
        $this->login();

        $response = $this->appRun('GET', '/bookmarks', server: [
            'QUERY_STRING' => 'page=2',
        ]);

        $this->assertResponseCode($response, 301, '/read/later?page=2');
    }
}
